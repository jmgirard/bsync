...
This file is the **single source of truth for milestone history**. Each
completed milestone has one detailed `M<N> (done)` entry here, in
numeric order with **no gaps**. To avoid the redundancy that used to
live in `CLAUDE.md`:

- **Detailed milestone narrative** (what shipped, durable decisions,
  deviations, final test count and `R CMD check` status) → this file.
- **One-line index per milestone** → `CLAUDE.md` “## Completed
  milestones”.
- **User-facing changes** → `NEWS.md`.
- **Design-contract changes** → `DESIGN.md` §1–14 (its §15 is a forward
  roadmap plus a pointer back here, **not** a second copy of this log).

New milestones begin at **M1**; substantial functionality predates
formal tracking (see Baseline).
...
A read of the baseline surfaced the defects M1–M3 address (see
`DESIGN.md` §14).
