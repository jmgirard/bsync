...
test_that("evaluate_signal_power returns expected structure for single vector", {
  ...
  res <- evaluate_signal_power(x, sample_rate = fs, threshold = 0.95, plot = TRUE)
  
  # Check structure against the newly refactored variable names
  expect_type(res, "list")
  expect_true(all(c("primary_cutoff_freq", "recommended_target_rate", "plot") %in% names(res)))
...
test_that("evaluate_signal_power handles multiple signals via data frame and list", {
  ...
  res_df <- evaluate_signal_power(df, sample_rate = fs, threshold = 0.95, plot = TRUE)
  
  expect_true(all(c("primary_cutoff_freq", "recommended_target_rate", "all_cutoffs", "summary_stats", "plot") %in% names(res_df)))
  ...
  expect_true(all(c("primary_cutoff_freq", "recommended_target_rate", "all_cutoffs", "summary_stats") %in% names(res_lst)))
...
test_that("evaluate_signal_power catches invalid inputs", {
  ...
  expect_error(evaluate_signal_power(c("a", "b"), 30), "must be a numeric vector, a list of numeric vectors, or a data frame")
  expect_error(evaluate_signal_power(data.frame(a = c("a", "b")), 30), "contains no numeric columns")
  expect_error(evaluate_signal_power(list(a = 1:10, b = "string"), 30), "All elements in the list")
  
  # Parameter errors
  expect_error(evaluate_signal_power(x, sample_rate = -5), "single positive number")
  expect_error(evaluate_signal_power(x, sample_rate = 30, threshold = 1.5), "between 0 and 1")
  expect_error(evaluate_signal_power(x, sample_rate = 30, plot = "TRUE"), "single logical value")
})
...
test_that("evaluate_signal_power handles missing data gracefully", {
  ...
  expect_error(evaluate_signal_power(x_bad, sample_rate = 10), "No signals contained enough non-missing data")
